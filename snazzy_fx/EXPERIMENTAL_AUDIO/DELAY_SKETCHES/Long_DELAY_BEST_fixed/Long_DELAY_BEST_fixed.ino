/* FIXED COPY, 24 Sep 2026, of Long_DELAY_BEST (the original is left as
   it was). What was wrong and what changed:

   The delay knob ran backwards. The code read the buffer delayLength
   samples ahead of where it writes, which in a ring buffer means
   (buffer size - delayLength) samples behind. So turning A3 up made the
   delay shorter. It now reads delayLength samples behind: turn A3 up
   for a longer delay. Nothing else is changed.

   Controls: A3 = delay time (up to about 60ms), A1 = feedback,
   A2 = audio in (keep the level low). A0 isn't read.
*/
