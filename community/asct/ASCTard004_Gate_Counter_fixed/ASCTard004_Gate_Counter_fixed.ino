...
//
//  Created:  18/03/2013
//  Modified: 24 Sep 2026  Fixed copy of ASCTard004_Gate_Counter (the
//            original is left as it was). What was wrong:
//
//            The counters were advanced with "count = count++;".
//            In C++ that reads count, bumps it, then writes the old
//            value back, so count never changes. The compiler
//            spotted that and removed the step entirely. The result
//            was that a channel only did anything with its count
//            set to zero (it fired, or toggled, on every clock) and
//            never fired at any other setting. Both lines are now
//            "count++;".
//
//            With that fixed, a channel fires (count mode) or flips
//            (toggle mode) every N+1 clocks, where N is 0 to 63 from
//            A2/A3: so every 1 to 64 clocks.
//
//  ============================================================
//
...
int counter(int input){
  if(counterval[input] >= countermax[input]){
     ...
     count++;   // was "count = count++;", see the note at the top
